...
def get_holidays_from_db(message):
    ...
    additional_condition = f"and week_num={day_idx}" if not is_last else \
                           f"and is_last={is_last}"
    ...
    holidays_from_db = '\n'.join(fetched) if fetched else \
                       'Сиводня праздников нет! Пойду сделаю омлет...'
    bot.send_message(
        chat_id=chat_id,
...
def get_wiki_holiday(message):
    ...
    month_ru = db_conn.query(
        f"select month_name from months where id={month};"
    )[0][0]
    holidays_list = []
    wiki_host = 'https://ru.wikipedia.org/'
    page = requests.get(
        f'{wiki_host}wiki/Категория:Праздники_{day}_{month_ru}',
        timeout=60
    )
    try:
